...
object AppModule {
    ...
    @Provides
    @Singleton
    fun providesDessertService(retrofit: Retrofit): DessertService = retrofit.create(DessertService::class.java)


    @Provides
    fun provideDao(db: Database) = db.dessertDao()

    @Singleton
    @Provides
    fun provideRoomInstace(@ApplicationContext ctx: Context)
    = Room.databaseBuilder(ctx, Database::class.java,Constants.DB_NAME)
